...
public class Player_Controller : MonoBehaviour
{
    ...
    private void UpdateMovementState()
    {
        ...
        bool canRun = CanRun();
        bool isMovementInput = _playerInputReader.MovementInput != Vector2.zero;             //order
        bool isMovingLaterally = IsMovingLaterally();                                            //matters
        bool isSprinting = _playerInputReader.SprintToggledOn && isMovingLaterally;          //order
        bool isWalking = isMovingLaterally && (!canRun || _playerInputReader.WalkToggledOn); //matters
    ...
    {
        _cameraRotation.x += lookSenseH * _playerInputReader.LookInput.x;
        _cameraRotation.y = Mathf.Clamp(_cameraRotation.y - lookSenseV * _playerInputReader.LookInput.y, -lookLimitV, lookLimitV);

        _playerTargetRotation.x += transform.eulerAngles.x + lookSenseH * _playerInputReader.LookInput.x;
    ...
    private bool CanRun()
    {
        ...
        return _playerInputReader.MovementInput.y >= Mathf.Abs(_playerInputReader.MovementInput.x);
    }
    #endregion
}